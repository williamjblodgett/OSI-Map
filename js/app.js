...
(function loadSpaceWeather() {
  ...
  function kpInfo(kp) {
    kp = parseFloat(kp) || 0;
    if (kp < 1)  return ['Quiet',             'quiet'];
    if (kp < 4)  return ['Unsettled',          'quiet'];
    if (kp < 5)  return ['Active',             'mod'];
    if (kp < 6)  return ['G1 Minor Storm',     'mod'];
    if (kp < 7)  return ['G2 Moderate Storm',  'high'];
    if (kp < 8)  return ['G3 Strong Storm',    'high'];
